...
AnimatedObject::AnimatedObject(string p, glm::vec3 ori, float esc, glm::vec3 pos, glm::vec3 u, glm::vec3 dir, GLuint sh) : Entity(pos, u, dir, sh)
{
    path = p;
    orientation = ori;
    scale = esc;
...
GLuint AnimatedObject::getShaderProgram()
{
...
void AnimatedObject::draw()
{
    ...
    glUniform1f(glGetUniformLocation(shaderProgram, "point_light.linear"), 0.007);	//0.14 0.09  0.07  0.045  0.027  0.022  0.014  0.007  0.0014 -	разное расстояние затухания
    glUniform1f(glGetUniformLocation(shaderProgram, "point_light.quadratic"), 0.0002);//0.07 0.032 0.017 0.0075 0.0028 0.0019 0.0007 0.0002 0.000007	расстояние -->
...
{
    model->initShaders(shaderProgram);
